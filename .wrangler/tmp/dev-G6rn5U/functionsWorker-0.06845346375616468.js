// .wrangler/tmp/bundle-J3kdsX/checked-fetch.js
...
// .wrangler/tmp/pages-GuWpux/functionsWorker-0.06845346375616468.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-yPj4kX/checked-fetch.js"() {
...
var init_wrangler_modules_watch = __esm({
  "wrangler-modules-watch:wrangler:modules-watch"() {
    init_functionsRoutes_0_040803648520392644();
    init_checked_fetch();
...
async function onRequestPost(context) {
  ...
  if (contentType2 != null) {
    ...
    if (registerData.action == 2) {
      ...
      const query2 = context.env.DB.prepare(`SELECT * FROM pragma_database_list;
`);
      const queryResult2 = await query2.all();
      ...
      if (queryResult.results.length > 0) {
        ...
        if (user.isAdmin == 1) {
          const query22 = context.env.DB.prepare(`SELECT COUNT(*) as total from property where isDeleted = 0`);
          const queryResult22 = await query22.first();
          user.foreignCount = queryResult22.total;
        } else {
          const query22 = context.env.DB.prepare(`SELECT COUNT(*) as total from property_owner where userId = ${user.id} and isDeleted = 0 `);
          const queryResult22 = await query22.first();
          user.foreignCount = queryResult22.total;
        }
...
async function onRequestGet3(context) {
  ...
  let query;
  let theResults = [];
  for (var i2 = 0; i2 < theData.length; ++i2) {
    let theSQL = `SELECT id,name from ${theData[i2].table}`;
    if (theData[i2].foreignKey != "") {
      theSQL = theSQL + ` where ${theData[i2].foreignKey} = ${theData[i2].value}`;
    ...
    let theJson = { "table": theData[i2].table, "key": theData[i2].key, "theData": queryResults.results };
...
async function onRequestGet4(context) {
  ...
  if (fieldsArray[0] != "") {
    for (var i2 = 0; i2 < queryResults.results.length; ++i2) {
      for (var i22 = 0; i22 < fieldsArray.length; ++i22) {
        if (fieldsArray[i22] == queryResults.results[i2].name)
          finFields.push(queryResults.results[i2]);
      }
    }
  } else {
...
var routes;
var init_functionsRoutes_0_040803648520392644 = __esm({
  "../.wrangler/tmp/pages-GuWpux/functionsRoutes-0.040803648520392644.mjs"() {
    init_account();
    init_email();
    init_image();
    init_lookUp();
...
// .wrangler/tmp/bundle-J3kdsX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-J3kdsX/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.06845346375616468.js.map
